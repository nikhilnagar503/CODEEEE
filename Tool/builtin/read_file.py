...
class ReadFileParams(BaseModel):
    ...
    limit : int | None  = Field(None, ge=1, description="maximum number of lines to read, default is None which means read the whole file")
...
class ReadFileTool(Tool):
    name = "read_file"
    description = (
        "Read the content of a text file and return it with line numbers. "
        "For large files, use offset and limit to read a specific portion. "
        "Cannot read binary files or files larger than 10 MB."
    )
    ...
    async def execute(self, invocation: ToolInvocation) -> ToolResult:
        ...
        try :
            ...
            token_count = count_tokens(output, None)
